...
import { getServerCSVLine, writeLinesToFile } from '@/home/tools/utilities';
import { SERVER_KEYS } from '@/home/data/enums';
...
export async function main(ns: NS) {
  ...
  const serverStrings = crackedHosts.map((host) => getServerString(host, ns));
  serverStrings.unshift(SERVER_KEYS.join(', '));
  writeLinesToFile('/data/cracked_hosts.txt', serverStrings, true, ns);
}
...
function getServerString(host: string, ns: NS): string {
  const server: Server = ns.getServer(host);
  return getServerCSVLine(server);
}
